...
if [ ! -d $MYSQL_DATADIR ]; then
    ...
    mysql --defaults-file=/etc/mysql/debian.cnf -e "CREATE DATABASE ${MIAB_SQL_DB_NAME};" >> /dev/null

    # Import our preconfigured database to MySQL
    mysql --defaults-file=/etc/mysql/debian.cnf ${MIAB_SQL_DB_NAME} < conf/mailinabox_init.sql >> /dev/null
fi
...
mysql --defaults-file=/etc/mysql/debian.cnf -e "GRANT ALL PRIVILEGES ON ${MIAB_SQL_DB_NAME}.* TO '${MIAB_SQL_DB_USER}'@'%' IDENTIFIED BY '${MIAB_SQL_DB_PW}';" >> /dev/null
